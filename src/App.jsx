  ...
  return (
    <div className="ms-root">
      ...
      <div className="ms-shell">
        ...
        <p className="ms-sub">Understand your habits. Keep the good. Change what hurts.</p>
        ...
        {tab === "explore" && (
          <div className="ms-grid">
            <nav className="ms-dock">
              <input className="ms-search" placeholder="Search apps…" value={search} onChange={(e) => setSearch(e.target.value)} />
              {filteredCategories.map((group) => (
                <React.Fragment key={group.cat}>
                  <div className="ms-cat-label">{group.cat}</div>
                  {group.apps.map((a) => (
                    <button key={a.id} className={"ms-dock-item" + (a.id === selected ? " active" : "")} onClick={() => setSelected(a.id)}>
                      <span className="ms-dock-icon"><svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.7">{a.icon}</svg></span>
